...
function verifySession(event){
 ...
 const ex=crypto.createHmac('sha256',secret).update(p).digest('base64url');if(s.length!==ex.length||!crypto.timingSafeEqual(Buffer.from(s),Buffer.from(ex)))return false;
