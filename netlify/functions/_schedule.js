const TZ='Europe/Moscow',S={0:null,1:['10:00','17:00'],2:['10:00','17:00'],3:['10:00','17:00'],4:['10:00','17:00'],5:['10:00','17:00'],6:['10:00','16:00']};
const mins=t=>{const m=/^(\d{2}):(\d{2})$/.exec(String(t||''));if(!m)return null;const h=+m[1],n=+m[2];return h<24&&n<60?h*60+n:null};
function dateInfo(s){const m=/^(\d{4})-(\d{2})-(\d{2})$/.exec(String(s||''));if(!m)return null;const y=+m[1],mo=+m[2],d=+m[3],x=new Date(Date.UTC(y,mo-1,d));return x.getUTCFullYear()===y&&x.getUTCMonth()===mo-1&&x.getUTCDate()===d?{d,x}:null}
...
module.exports={TZ,S,mins,dateInfo,now,today,validate,type};