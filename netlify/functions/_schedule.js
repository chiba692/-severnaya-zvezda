...
function nextFirstSaturday(from=today()){for(let i=0;i<45;i++){const ds=addDays(from,i),di=dateInfo(ds);if(di&&di.x.getUTCDay()===6&&di.d<=7)return ds}return null}
function generateSlots(date){const di=dateInfo(date);if(!di)return[];const sch=S[di.x.getUTCDay()];if(!sch)return[];const a=mins(sch[0]),b=mins(sch[1]),step=slotMinutes(date),out=[];for(let m=a;m<b;m+=step)out.push(`${String(Math.floor(m/60)).padStart(2,'0')}:${String(m%60).padStart(2,'0')}`);return out}
module.exports={TZ,S,SLOT,TRAUMA_SLOT,mins,dateInfo,now,today,validate,type,slotMinutes,addDays,nextFirstSaturday,generateSlots};
