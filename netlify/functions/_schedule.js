...
function generateSlots(date){const di=dateInfo(date);if(!di)return[];const sch=S[di.x.getUTCDay()];if(!sch)return[];const a=mins(sch[0]),b=mins(sch[1]),out=[];for(let m=a;m<b;m+=SLOT)out.push(`${String(Math.floor(m/60)).padStart(2,'0')}:${String(m%60).padStart(2,'0')}`);return out}
module.exports={TZ,S,SLOT,mins,dateInfo,now,today,validate,type,addDays,nextFirstSaturday,generateSlots};
